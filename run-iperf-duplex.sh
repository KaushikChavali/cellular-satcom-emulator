...
function _osnd_moon_iperf_server_dl_start() {
    ...
    tmux -L ${TMUX_SOCKET} new-session -s iperf-dl -d "sudo ip netns exec osnd-moon-cl bash"
    sleep $TMUX_INIT_WAIT
    tmux -L ${TMUX_SOCKET} send-keys -t iperf-dl "${IPERF_BIN} -s -p 5201 -i ${REPORT_INTERVAL} > \"${output_dir}/${run_id}_iperf_dl_server.log\" 2>&1" Enter
}
...
function _osnd_moon_iperf_server_ul_start() {
    ...
    sleep $TMUX_INIT_WAIT
    tmux -L ${TMUX_SOCKET} send-keys -t iperf-ul "${IPERF_BIN} -s -p 4242 -i ${REPORT_INTERVAL} > \"${output_dir}/${run_id}_iperf_ul_server.log\" 2>&1" Enter
}
...
# osnd_moon_measure_iperf_tcp_duplex_metrics(scenario_config_name, output_dir, pep=false, route, run_cnt=12)
...
function osnd_moon_measure_iperf_tcp_duplex_metrics() {
    ...
    for i in $(seq $run_cnt); do
        ...
        _osnd_moon_iperf_server_stop "iperf-dl" "osnd-moon-cl"
...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
    ...
    if [[ "$@" ]]; then
        osnd_moon_measure_iperf_tcp_duplex_metrics scenario_config "$@"
    else
        osnd_moon_measure_iperf_tcp_duplex_metrics scenario_config "." 0 1
    fi
fi
